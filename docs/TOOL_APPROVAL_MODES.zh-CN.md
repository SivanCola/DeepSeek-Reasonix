...
- 计划确认有三个直接动作：「开始执行 / 修改计划 / 暂不执行，退出计划模式」，点击一次或按对应数字键就会执行，不再需要第二次“确认”。退出操作会拒绝待处理计划并返回普通模式，不会启动执行回合。
- `Esc` 停止当前任务。
...
- 失败后的诊断和恢复继续自动推进。同一操作连续失败 3 次时，Auto 只停止重复该操作；相关编辑、替代实现和不同的验证方式仍可继续，不会把整个会话的写入工具锁住。Reviewer 的拒绝预算同样只针对当前候选方案。
- “换个方案”会结束当前失败恢复回合，让 Auto 从替代方案继续；新的普通用户回合也会自动清除上一回合的技术失败锁。目标模式的连续交付仍保留目标内恢复上下文。
...
- Auto Guard 不提供写入工具白名单，也不需要用户管理这类白名单；它只阻止当前反复失败的具体操作，基础能力边界仍由权限策略和 sandbox 决定。
