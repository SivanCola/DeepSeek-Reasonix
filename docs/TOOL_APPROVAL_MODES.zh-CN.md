...
- 建立和更新普通任务计划都走快速路径，模型直接使用 `todo_write`；Auto
  不再插入恢复 reviewer 或计划转换卡。
- 工具失败是一条执行事实，不是整个任务的权限边界。后续调用分别执行自己的权限、
  sandbox 和文件版本检查；重试相同操作也不会进入恢复审查。
- 超时只会获得简短的失败路径提示，要求先检查当前状态和部分执行结果再重试；不会要求用户重置 Auto、切换模式或重启会话。
- 完全相同的连续工具调用只在第 3、5、8 次得到提醒，不会禁用工具，也不会建立
  Episode 或恢复暂停。
- 无人值守环境遇到真正的计划决策时会 fail closed，不会静默代选。
- 无头 Ask/Auto/DontAsk 遇到嵌套或间接 Bash 也会 fail closed，除非已有完全相同的 `Bash=<literal>` 精确授权。
- Ask、Auto 与 YOLO 保持各自权限语义；不存在额外的恢复安全开关。
...
- 已退役的 Auto Guard 不再包含运行时门禁、操作级停止、Episode 预算或重置流程；基础能力边界仍由权限策略和 sandbox 决定。
