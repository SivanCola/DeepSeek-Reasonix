...
- Yolo 只跳过工具权限审批（含交互式 `remember`/`forget`），不会跳过计划确认和 ask 问题。
- Yolo 不会绕过后端的 `deny` 规则、系统权限、工作区限制或沙箱限制。
- 计划确认、沙箱逃逸和受管配置写入仍然会等待你处理。Ask/Auto 下其余记忆变更仍需确认；
  交互式 Yolo 不再为 `remember`/`forget` 弹窗，除非命中显式 deny。MCP 授权发生在用户安装或项目配置声明时，不需要额外项目身份确认；
