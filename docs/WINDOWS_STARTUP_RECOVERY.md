...
First launch stays hidden until the service is ready or startup fails, so a healthy boot does not flash the diagnostic wait page. A second click while still starting stays hidden or focuses an existing window; it does not open the wait page. Failed startups still show the recovery page. Second launches of a ready app show the current window, including minimized/tray windows. Build/contract mismatch pages recommend a complete package and do not expose a retry loop. Startup readiness has a 30-second deadline. The existing business close policy still controls normal tray hiding.
