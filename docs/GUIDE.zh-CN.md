...
以 `先规划` / `plan first` 开头会强制规划，`直接改` / `just do it` 则直达 Executor；
只要求方案的请求停在宿主审批边界，不会自动开始执行。阶段详情会记录不含用户原文的
route、depth 与 reason code，便于诊断。
