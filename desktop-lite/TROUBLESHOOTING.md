...
## 11. 只监听一次性事件，UI 会永远停在 "starting…"

**症状**：应用启动后状态卡在 "starting…"、输入框不解锁，但日志无任何错误——**时有时无**。

**根因**：`startup` 里的会话装配是异步的，完成后发一个 `ready` 帧。若 webview 尚未挂载
完成、还没调用 `EventsOn` 订阅，这个**一次性事件就永久丢失**了，UI 再也等不到。装配快
慢受磁盘/MCP 影响，所以表现为偶发。

**修法**：状态要**可查询**，不能只可订阅。`App.Ready()` 暴露当前状态，前端挂载后轮询
直到就绪，同时保留事件监听（谁先到算谁）。

**通则**：凡是"启动期一次性事件 + 前端异步挂载"的组合都有这个坑。设计时问一句——
**如果这个事件在监听器注册前就发了，会怎样？**

## 12. 会话被替换后，旧轮次的完成会污染新会话状态
