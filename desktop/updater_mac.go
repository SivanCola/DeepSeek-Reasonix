...
func macAppBundleForExecutable(exe string) (string, error) {
	...
	// Preserve the installed path spelling (/var versus /private/var and an app
	// path reached through a user-visible alias), while still requiring the link
	// chain to resolve. A compatibility service link remains attributable to the
	// .app containing the link; a launcher outside the app may use its in-bundle
	// resolved target.
