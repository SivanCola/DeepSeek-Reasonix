...
import type { ReasonixCommandStatus } from "../../lib/types";
import type { Balance, Settings as SettingsType, UsageStats } from "../compat-types";
...
import { openUrl } from "../tauri-compat";
...
function PageGeneral({
  ...
  const [commandStatus, setCommandStatus] = useState<ReasonixCommandStatus | null>(null);
  const [commandBusy, setCommandBusy] = useState(false);
  const [commandMsg, setCommandMsg] = useState<string | null>(null);
  const lang = useLang();
  const reloadCommandStatus = async () => {
    setCommandStatus(await app.ReasonixCommandStatus().catch(() => null));
  };
  useEffect(() => {
    void reloadCommandStatus();
  }, []);
  const installCommand = async () => {
    ...
    setCommandMsg(null);
    try {
      const result = await app.InstallReasonixCommand();
      setCommandMsg(
        t(result.action === "installed" ? "settings.commandInstalledToast" : "settings.commandUpdatedToast", {
          path: result.path,
        }),
      );
    } catch (err) {
      setCommandMsg(String((err as Error)?.message ?? err));
    } finally {
      await reloadCommandStatus();
  ...
  const visibleThemeStyles = THEME_STYLES.filter((style) => themeForStyle(style) === theme);
    ...
    <>
      <section className="section">
        ...
        <div className="appearance-picker">
          ...
          <div className="appearance-picker-copy">
            ...
            {visibleThemeStyles.map((style) => (
              ...
              >
                <span className="style-card-head">
                  ...
                  {themeStyle === style ? (
                    <span className="style-mode" aria-hidden="true">
                      <I.check size={13} />
                    </span>
                  ) : null}
                </span>
        ...
        <div className="setting-row">
          ...
          <div className="seg-ctrl">
            {getSupportedLangs().filter((code) => code === "en" || code === "zh-CN").map((code) => (
      ...
      <section className="section">
        <div className="stitle">{t("settings.editorSection")}</div>
        <div className="setting-row">
          <div className="l">
...
function CommandInstallRow({
  status,
  busy,
  message,
  onInstall,
}: {
  status: ReasonixCommandStatus | null;
  busy: boolean;
  message: string | null;
  onInstall: () => Promise<void>;
}) {
  const state = status?.state ?? "missing";
  const path = status?.path ?? "/usr/local/bin/reasonix";
  const hint =
    state === "installed"
      ? t("settings.commandHintInstalled", { path })
      : state === "needsUpdate"
        ? t("settings.commandHintUpdate", { path })
        : state === "foreign"
          ? t("settings.commandHintForeign")
          : state === "unsupported"
            ? t("settings.commandHintUnsupported")
            : t("settings.commandHint");
  const label = busy
    ? t("settings.commandInstalling")
    : state === "installed"
      ? t("settings.commandInstalled")
      : state === "needsUpdate"
        ? t("settings.commandUpdate")
        : state === "foreign"
          ? t("settings.commandExists")
          : state === "unsupported"
            ? t("settings.commandUnsupported")
            : t("settings.commandInstall");
  const disabled = busy || state === "installed" || state === "foreign" || state === "unsupported";
  return (
    <div className="setting-row">
      <div className="l">
        <div className="n">{t("settings.commandTitle")}</div>
        <div className="h">{message ?? hint}</div>
      </div>
      <button type="button" className="btn primary" disabled={disabled} onClick={() => void onInstall()}>
        {label}
      </button>
    </div>
  );
...
function PageAbout() {
  const { status, check, apply, openDownload } = useUpdater();
  const [version, setVersion] = useState("");

  useEffect(() => {
    void app.Version().then(setVersion).catch(() => undefined);
  }, []);
  ...
  return (
    <section className="section">
      ...
      <div className="about-settings-card">
        <div className="about-settings-brand">
          <div className="about-settings-mark" />
          <div>
            <div className="about-settings-name">Reasonix</div>
            <div className="h">{t("about.tagline")}</div>
          </div>
        </div>
        <div className="setting-row">
          <div className="l">
            <div className="n">{t("about.version")}</div>
            <div className="h">{version || "..."}</div>
          </div>
          <button type="button" className="btn" onClick={() => void openUrl(REPO_URL)}>
            <I.link size={12} />
            {t("about.repo")}
          </button>
        </div>
        <div className="setting-row">
          <div className="l">
            <div className="n">{t("settings.updateSection")}</div>
            <div className="h">{updateStatusText(status, version)}</div>
          </div>
          <div className="about-settings-actions">
            ...
            {status.kind === "available" ? (
              <button type="button" className="btn primary" onClick={() => apply(status.info)}>
                <I.download size={12} />
                {status.info.canSelfUpdate ? t("settings.updateNow") : t("settings.updateDownload")}
              </button>
            ) : null}
            {status.kind === "error" ? (
        ...
        {status.kind === "available" && !status.info.canSelfUpdate ? (
          <div className="h about-settings-note">{t("settings.updateMacHint")}</div>
        ) : null}
        {status.kind === "available" && status.info.notes ? (
          <div className="about-settings-notes">{status.info.notes}</div>
        ) : null}
      </div>
    </section>
  );
...
function updateStatusText(status: ReturnType<typeof useUpdater>["status"], version: string): string {
  switch (status.kind) {
    case "idle":
      return t("settings.updateIdle", { version: version || "..." });
    case "checking":
      return t("about.checking");
    case "upToDate":
      return t("about.upToDate", { version: status.current || version || "..." });
    case "available":
      return t("about.updateAvailable", { version: status.info.latest });
    case "downloading":
      return t("settings.updateDownloading", {
        done: formatMb(status.received),
    ...
    case "applying":
      return t("settings.updateApplying");
    case "done":
      return t("settings.updateDone");
    case "error":
      return t("about.checkFailed", { message: status.message });
  }
}
