...
export function RemotePanel({ onClose }: { onClose: () => void }) {
  ...
  const busy = status?.state === "connecting" || status?.state === "reconnecting" || status?.state === "pending_hostkey" || status?.state === "pending_secret" || status?.state === "pending_provider_trust";
