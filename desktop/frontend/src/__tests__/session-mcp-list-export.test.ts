...
import { mcpListAttribution, sessionItemsToJson } from "../lib/sessionExport";
...
describe("session export MCP list attribution", () => {
  it("counts shared host, disk cache, and remote tools/list sources", () => {
    const items: Item[] = [
      ...
      { kind: "notice", id: "4", level: "info", text: "remote tools/list completed" },
    ];
    const got = mcpListAttribution(items);
    assert.deepEqual(got, { sharedHost: 1, diskCache: 1, remote: 2, networkCalls: 1 });
