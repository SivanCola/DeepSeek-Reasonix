...
await waitFor(() => Boolean(document.querySelector(".context-panel__metric-tooltip-target[aria-label='运行时间: 42秒']")));

const shortCard = document.querySelector<HTMLElement>(".context-panel__metric-tooltip-target[aria-label='运行时间: 42秒']");
...
eq(tooltip?.textContent?.trim(), "运行时间: 42秒", "hovering a short duration reveals the tooltip");
...
await waitFor(() => Array.from(document.querySelectorAll(".context-panel__mini-stat strong")).some((node) => node.textContent?.trim() === "19时49分"));

const longLabels = Array.from(document.querySelectorAll(".context-panel__mini-stat strong")).map((node) => node.textContent?.trim());
ok(longLabels.includes("19时49分"), "time card body keeps only the largest two units");

const longCard = document.querySelector<HTMLElement>(".context-panel__metric-tooltip-target[aria-label='运行时间: 19时49分1秒']");
...
eq(tooltip?.textContent?.trim(), "运行时间: 19时49分1秒", "hovering a long duration reveals full segmented detail");
