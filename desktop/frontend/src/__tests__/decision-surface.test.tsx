...
// Write-access approval: four scopes, home warning, merged permission hint.
{
  const dom = installDom();
  const root = createRoot(document.getElementById("root")!);
  const answers: Array<[boolean, boolean, boolean]> = [];
  ...
  const writeActions = [...document.querySelectorAll(".prompt-shelf__actions .prompt-action")] as HTMLButtonElement[];
  eq(writeActions.length, 4, "write-access approval has four options");
  ok(writeActions[0].textContent?.includes("Allow once"), "first option is allow once");
  ok(writeActions[1].textContent?.includes("this session"), "second option is session grant");
  ok(writeActions[2].textContent?.includes("project"), "third option persists to the project");
  ok(writeActions[3].textContent?.includes("Deny"), "fourth option is deny");

  await act(async () => {
    writeActions[1].click();
    await flushTimers();
  });
  eq(answers.length, 0, "clicking session only selects");
  const writeConfirm = document.querySelector(".decision-confirm-bar__confirm") as HTMLButtonElement;
  await act(async () => {
    writeConfirm.click();
    writeConfirm.click();
    await flushTimers(220);
