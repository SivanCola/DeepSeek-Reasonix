...
/**
 * Records a child this tab published, keeping the first id it recorded for the
 * turn: a repeat fork reopens that child instead of creating a second one.
 */
export function rememberForkChild(children: Record<string, string>, turnId: string, sessionId: string): Record<string, string> {
  return children[turnId] === sessionId ? children : { ...children, [turnId]: sessionId };
}
...
export interface RemoteForkTurnApi {
  ...
  /** Forgets every child of the session the tab just adopted. */
...
export function useRemoteForkTurn(
  ...
  const [forkChildren, setForkChildren] = useState<Record<string, string>>({});
  ...
  const forkTurn = useCallback(async (turnId: string): Promise<string | undefined> => {
    ...
    setPromptError("");
    const outcome = await requestRemoteForkTurn(bindings, tabId, turnId, forkChildren[turnId]);
    if (outcome.kind === "created") return outcome.sessionId;
    setPromptError(outcome.failure);
    return undefined;
  }, [bindings, forkChildren, setPromptError, tabId]);

  const rememberUnopenedFork = useCallback(
    (turnId: string, sessionId: string) => setForkChildren((current) => rememberForkChild(current, turnId, sessionId)),
    [],
  );
  const resetForkChildren = useCallback(() => setForkChildren({}), []);
