...
export function useController() {
  ...
  const activateTopic = useCallback(async (scope: string, workspaceRoot: string, topicId: string, sessionPath = ""): Promise<TabMeta> => {
    ...
    void loadSessionDataForTab(meta.id, true, "open-topic", { placeholderItems: prevItems })
      ...
      .catch(() => {});
    return meta;
  }, [beginActiveNavigation, confirmBackendActiveTab, dispatchRuntimeStatusForTab, dispatchTo, disposeComposerProfileState, invalidateProviderStateForTab, loadSessionDataForTab, reconcileTabRuntime]);
