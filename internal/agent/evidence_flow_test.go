...
func TestEvidenceFlowAllowsThreeOrderedCompleteStepSignoffs(t *testing.T) {
	...
	prov := &scriptedProvider{name: "p", turns: [][]provider.Chunk{
		{
			toolCallChunk("t", "todo_write", `{"todos":[
	...
	if got := len(toolResults(a.sess.conversation, "complete_step")); got != 3 {
		t.Fatalf("complete_step results = %d, want three in one batch", got)
	}
	if prov.call != 3 {
		t.Fatalf("provider calls = %d, want setup, one batch, final answer", prov.call)
	}
	for i, todo := range a.CanonicalTodoState() {
		if todo.Status != "completed" {
			t.Fatalf("canonical todo %d = %+v, want completed", i+1, todo)
...
func TestEvidenceFlowBatchCompletesPhaseChildrenThenPhaseHeader(t *testing.T) {
	...
	prov := &scriptedProvider{name: "p", turns: [][]provider.Chunk{
		{toolCallChunk("t", "todo_write", `{"todos":[
			...
			{"content":"child one","status":"in_progress","level":1},
			{"content":"child two","status":"pending","level":1},
			{"content":"Finalize","status":"pending","level":0}
		]}`), {Type: provider.ChunkDone}},
		{
