...
func TestTargetedVerificationRecoveryPreservesEvidenceOnce(t *testing.T) {
	...
	if !ok {
		t.Fatal("recovery turn lost the prior write receipt")
	}
	if !a.task.ledger.HasSuccessfulVerificationCommandAfter(writer) {
		t.Fatal("recovery turn did not retain the prior write before the recognized check")
	}
}
...
func TestTargetedVerificationRecoverySurvivesAgentRebuild(t *testing.T) {
	...
	// The canonical tool call stores the payload once; the recovery checkpoint
	...
	if !reloaded.PrepareFinalReadinessRecovery() {
		t.Fatal("rebuilt agent did not restore the pending readiness checkpoint")
	}
	if reloaded.PrepareFinalReadinessRecovery() {
		t.Fatal("rebuilt recovery authorization must remain one-shot")
	}
