...
func (a *Agent) compact(ctx context.Context, trigger, instructions string, force bool) error {
	...
	if !force && !foldEconomics(foldRegion) {
		return nil
	}
	...
	summary, err := a.summarize(ctx, foldRegion, instructions)
	if err != nil {
		a.emitCompactionAborted(trigger)
		return err
	}
	...
	a.sink.Emit(event.Event{Kind: event.CompactionDone, Compaction: event.Compaction{
		Trigger: trigger, Messages: len(foldRegion), Summary: summary, Archive: archived,
	}})
	return nil
}
