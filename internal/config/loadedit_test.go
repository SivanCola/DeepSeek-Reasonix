...
func TestLoadMigratesLegacyMCPTiersFromMCPTOML(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
tier = "lazy"
`
	if err := os.WriteFile(UserMCPConfigPath(), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	updated, err := os.ReadFile(UserMCPConfigPath())
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(updated), "\ntier") {
		t.Fatalf("legacy tier lines should be removed from mcp.toml:\n%s", updated)
	}
}
