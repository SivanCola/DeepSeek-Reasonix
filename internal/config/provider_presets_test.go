...
func TestDeepSeekAnthropicPresetIsOptionalAndModelScoped(t *testing.T) {
	...
	if entry.Kind != "anthropic" || entry.BaseURL != deepSeekAnthropicBaseURL || entry.Default != "deepseek-v4-flash" || entry.Thinking != "enabled" || !entry.WebSearch || entry.Vision || entry.APIKeyEnv != "DEEPSEEK_API_KEY" {
...
func TestDeepSeekResponsesPresetMatchesOfficialSupport(t *testing.T) {
	...
	if !entry.WebSearch || entry.Vision || entry.VisionModels != nil {
		t.Fatalf("deepseek responses capabilities = web_search:%t vision:%t vision_models:%v", entry.WebSearch, entry.Vision, entry.VisionModels)
	}
}
