...
// CurrentConfigVersion is the QQ Bot V2 schema generation. Default() remains
// compatible with the current general config migration tests; bot settings
// that are normalized to [[bot.connections]] are promoted to this version.
...
func NormalizeLegacyQQConnection(c *Config) bool {
	...
	for _, conn := range c.Bot.Connections {
		...
		if provider == "qq" || provider == "onebot" {
			return false
		}
	}
