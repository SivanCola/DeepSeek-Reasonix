...
func TestToolCallMutatesForDeliveryProfile(t *testing.T) {
	tests := []struct {
		...
		{name: "pure commit is state mutation", toolName: "bash", args: `{"command":"git commit -m checkpoint"}`, want: true},
		{name: "env prefixed test", toolName: "bash", args: `{"command":"GOROOT=/x go test ./..."}`},
