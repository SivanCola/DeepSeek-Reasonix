...
func (gw *BotGateway) handleMessage(ctx context.Context, binding AdapterBinding, msg InboundMessage) {
	...
	completeIngress := func() {
		if ingressID == "" || gw.cfg.IngressJournal == nil {
			return
		}
		if err := gw.cfg.IngressJournal.Complete(ingressID); err != nil {
...
func (gw *BotGateway) runTurn(ctx context.Context, adapter Adapter, key string, msg InboundMessage, cleanup func()) {
	...
	defer cancel()
	ingressID := gw.ingressIDForMessage(msg)
	if ingressID != "" {
		turnCtx = agent.WithHostTurnID(turnCtx, ingressID)
	}
	turnCtx = agent.WithHostTurnAccepted(turnCtx, func() {
		if ingressID != "" && gw.cfg.IngressJournal != nil {
			if err := gw.cfg.IngressJournal.MarkAccepted(ingressID, ingressID); err != nil {
				gw.logger.Warn("mark bot ingress accepted failed", "err", err)
			}
		}
		if gw.cfg.OnRemoteTurnAccepted != nil {
			gw.cfg.OnRemoteTurnAccepted(msg, state.ctrl.SessionPath(), input)
		}
	})
	if ingressID != "" && gw.cfg.IngressJournal != nil {
		defer gw.completeIngressMessage(msg)
	}
	...
	// 运行一轮对话
	err := gw.broker.SubmitTurn(turnCtx, TurnRequest{ID: gw.ingressIDForMessage(msg), SessionKey: key, Run: func(runCtx context.Context) error {
		return state.ctrl.RunTurn(runCtx, input)
	}})
...
func (gw *BotGateway) completeIngressMessage(msg InboundMessage) {
	if gw == nil || gw.cfg.IngressJournal == nil {
		return
	}
	if id := gw.ingressIDForMessage(msg); id != "" {
