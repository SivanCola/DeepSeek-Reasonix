...
"properties":{
  ...
  "scope":{"type":"string","enum":["project","global"],"description":"Where to persist config or copy skills. Reasonix stores MCP and skills globally; project is accepted as a legacy alias for global."},
...
func (t *installSourceTool) executeUninstall(req request) string {
	scope := req.Scope
	actions := []action{}
	skillCfgPath := t.skillConfigPath(scope)
	mcpCfgPath := t.mcpConfigPath(scope)
	...
	if path, ok := t.resolveSkillPath(req.Name, scope); ok {
		actions = append(actions, action{
			...
			Target:     path,
			Scope:      scope,
			ConfigPath: skillCfgPath,
			RiskLevel:  RiskLow,
		})
	} else if rootAction, ok := t.resolveRegisteredSkillRoot(req.Name, scope, skillCfgPath, cfg); ok {
...
func (t *installSourceTool) configPath(scope string) string {
	if scope == "global" {
		if t.home != "" {
			return filepath.Join(t.home, ".reasonix", "config.toml")
		}
	}
	return filepath.Join(t.root, "reasonix.toml")
}

func (t *installSourceTool) skillConfigPath(scope string) string {
	if scope == "global" {
		if t.home != "" {
			return filepath.Join(t.home, ".reasonix", "skills.toml")
		}
	}
	return t.configPath(scope)
}

func (t *installSourceTool) mcpConfigPath(scope string) string {
	if scope == "global" {
		if t.home != "" {
			return filepath.Join(t.home, ".reasonix", "mcp.toml")
		}
	}
	return t.configPath(scope)
}

func (t *installSourceTool) loadConfig() (*config.Config, error) {
	cfg := config.LoadForEdit(t.configPath("global"))
	if path := t.skillConfigPath("global"); path != "" {
		var top struct {
			Skills config.SkillsConfig `toml:"skills"`
		}
	...
	if path := t.mcpConfigPath("global"); path != "" {
		var top struct {
			Plugins []config.PluginEntry `toml:"plugins"`
		}
		if _, err := os.Stat(path); err == nil {
			if _, err := toml.DecodeFile(path, &top); err != nil {
				return nil, err
			}
			cfg.Plugins = top.Plugins
			for i := range cfg.Plugins {
				cfg.Plugins[i], _ = config.NormalizePluginCommandLine(cfg.Plugins[i])
			}
		}
	}
	return cfg, nil
