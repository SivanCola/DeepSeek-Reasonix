...
func TestIdleStatuslineIsCompact(t *testing.T) {
	...
	if !strings.Contains(plain, "⇧Tab ask/auto/plan · ^Y YOLO") {
...
func TestYoloStatuslineUsesDangerPill(t *testing.T) {
	...
	plain := bottomStatusPlain(content)
	if !strings.Contains(plain, "YOLO") || !strings.Contains(plain, "approvals skipped") || !strings.Contains(plain, "⇧Tab ask/auto/plan · ^Y YOLO") {
...
func TestPlanStatuslineUsesBluePill(t *testing.T) {
	...
	plain := bottomStatusPlain(content)
	if !strings.Contains(plain, "Plan") || !strings.Contains(plain, "ready") || !strings.Contains(plain, "⇧Tab ask/auto/plan · ^Y YOLO") {
...
func TestStatuslineCycleHintFollowsLanguage(t *testing.T) {
	...
	content := renderStatuslineView(t, false)
	plain := bottomStatusPlain(content)
	if !strings.Contains(plain, "Auto") || !strings.Contains(plain, "就绪") || !strings.Contains(plain, "⇧Tab 询问/自动/计划 · ^Y YOLO") {
		t.Fatalf("localized plan-toggle hint missing:\n%s", plain)
	}
	if strings.Contains(plain, "ready") || strings.Contains(plain, "⇧Tab ask/auto/plan · ^Y YOLO") {
...
func TestDesktopShortcutStatuslineUsesPlanToggleHint(t *testing.T) {
	...
	plain := bottomStatusPlain(content)
	if !strings.Contains(plain, "Ask") || !strings.Contains(plain, "⇧Tab ask/auto/plan · ^Y YOLO") {
