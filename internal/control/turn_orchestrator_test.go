...
func TestTurnOrchestratorAttachesTrustedPlannerMetadata(t *testing.T) {
	...
	if runner.meta.ExplicitPlanMode || !runner.meta.GoalActive || !runner.meta.ClosedLoop {
		t.Fatalf("planner metadata missing trusted host state: %+v", runner.meta)
	}
	if !runner.meta.PolicySet || !runner.meta.Policy.ClosedLoop() {
		t.Fatalf("planner metadata should carry a closed-loop TaskPolicy for an active Goal: %+v", runner.meta)
	}
