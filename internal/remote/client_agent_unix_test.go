...
func TestIdentitiesOnlySkipsUnrelatedAgent(t *testing.T) {
	pemBytes, authorized, err := sshtest.GenerateKeyPEM()
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cancel()
	if err := c.Start(ctx); err != nil {
		t.Fatalf("Start with IdentitiesOnly: %v", err)
	}
	defer c.Close()
	select {
	case <-accepted:
		t.Fatal("SSH agent was contacted despite IdentitiesOnly=yes")
	default:
	}
}
