...
export async function findReceipt(expected, token, fetcher = fetch) {
  ...
  const url = `https://api.github.com/repos/${expected.GITHUB_REPOSITORY}/actions/runs/${expected.GITHUB_RUN_ID}/artifacts?name=${name}&per_page=100`;
  const response = await fetcher(url, {
    headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28" },
    signal: AbortSignal.timeout(30000),
  });
  if (!response.ok) throw new Error(`cannot look up signing receipt (HTTP ${response.status}); refusing to resubmit`);
...
export function requireRecoverable(exists, attempt) {
  if (!/^[1-9][0-9]*$/.test(attempt ?? "")) throw new Error("invalid workflow attempt");
  if (!exists && attempt !== "1") {
...
async function main() {
  ...
  if (command === "prepare") {
    const exists = await findReceipt(expected, process.env.GH_TOKEN);
    requireRecoverable(exists, process.env.GITHUB_RUN_ATTEMPT);
